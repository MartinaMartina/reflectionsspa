...
function vp_pfui_add_meta_boxes($post_type) {
	if (post_type_supports($post_type, 'post-formats') && current_theme_supports('post-formats')) {
		// assets
		wp_enqueue_script('simple-elegant', vp_pfui_base_url().'js/admin.js', array('jquery'), VP_PFUI_VERSION);
		wp_enqueue_style('simple-elegant', vp_pfui_base_url().'css/admin.css', array(), VP_PFUI_VERSION, 'screen');

		wp_localize_script(
			'simple-elegant',
			'vp_pfui_post_format',
			array(
				'loading'      => esc_html__('Loading...', 'simple-elegant'),
				'wpspin_light' => admin_url('images/wpspin_light.gif'),
				'media_title'  => esc_html__('Pick Gallery Images', 'simple-elegant'),
				'media_button' => esc_html__('Add Image(s)', 'simple-elegant')
			)
		);
