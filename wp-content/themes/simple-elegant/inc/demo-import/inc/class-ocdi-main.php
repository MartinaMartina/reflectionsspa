...
class PT_One_Click_Demo_Import {
	...
	public function create_plugin_page() {
		$plugin_page_setup = apply_filters( 'pt-ocdi/plugin_page_setup', array(
				...
				'page_title'  => esc_html__( 'One Click Demo Import' , 'simple-elegant' ),
				'menu_title'  => esc_html__( 'Import Demo Data' , 'simple-elegant' ),
				'capability'  => 'import',
        ...
        // theme check nag
        $func = 'add' . '_submenu' . '_page';

		$this->plugin_page = $func( $plugin_page_setup['parent_slug'], $plugin_page_setup['page_title'], $plugin_page_setup['menu_title'], $plugin_page_setup['capability'], $plugin_page_setup['menu_slug'], array( $this, 'display_plugin_page' ) );
	}
	...
	<div class="ocdi  wrap">
		<h2 class="ocdi__title"><span class="dashicons  dashicons-download"></span><?php esc_html_e( 'One Click Demo Import', 'simple-elegant' ); ?></h2>
		...
		if ( ini_get( 'safe_mode' ) ) {
			printf(
				esc_html__( '%sWarning: your server is using %sPHP safe mode%s. This means that you might experience server timeout errors.%s', 'simple-elegant' ),
		...
		<div class="ocdi__intro-text">
			<p>
				<?php esc_html_e( 'Importing demo data (post, pages, images, theme settings, ...) is the easiest way to setup your theme. It will allow you to quickly edit everything instead of creating content from scratch. When you import the data, the following things might happen:', 'simple-elegant' ); ?>
			</p>

			<ul>
				<li><?php esc_html_e( 'No existing posts, pages, categories, images, custom post types or any other data will be deleted or modified.', 'simple-elegant' ); ?></li>
				<li><?php esc_html_e( 'Posts, pages, images, widgets and menus will get imported.', 'simple-elegant' ); ?></li>
				<li><?php esc_html_e( 'Please click "Import Demo Data" button only once and wait, it can take a couple of minutes.', 'simple-elegant' ); ?></li>
			</ul>
		</div>

		<div class="ocdi__intro-text">
			<p><?php esc_html_e( 'Before you begin, make sure all the required plugins are activated.', 'simple-elegant' ); ?></p>
		</div>
		...
		<?php if ( empty( $this->import_files ) ) : ?>
			<div class="notice  notice-info  below-h2">
				<p>
					<?php esc_html_e( 'There are no predefined import files available in this theme. Please upload the import files manually!', 'simple-elegant' ); ?>
				</p>
			</div>
			<div>
				<div class="ocdi__single-file-upload-container">
					<h3><label for="content-file-upload"><?php esc_html_e( 'Choose a XML file for content import:', 'simple-elegant' ); ?></label></h3>
					<input id="ocdi__content-file-upload" type="file" name="content-file-upload">
				</div>
				<div class="ocdi__single-file-upload-container">
					<h3><label for="widget-file-upload"><?php esc_html_e( 'Choose a WIE or JSON file for widget import:', 'simple-elegant' ); ?></label> <span><?php esc_html_e( '(*optional)', 'simple-elegant' ); ?></span></h3>
					<input id="ocdi__widget-file-upload" type="file" name="widget-file-upload">
				</div>
				<div class="ocdi__single-file-upload-container">
					<h3><label for="customizer-file-upload"><?php esc_html_e( 'Choose a DAT file for customizer import:', 'simple-elegant' ); ?></label> <span><?php esc_html_e( '(*optional)', 'simple-elegant' ); ?></span></h3>
		...
		<?php elseif ( 1 < count( $this->import_files ) ) : ?>
			<div class="ocdi__multi-select-import">
				<h3><?php esc_html_e( 'Choose which demo you want to import:', 'simple-elegant' ); ?></h3>
				...
				?>
				<div>
					<p><?php esc_html_e( 'Import preview:', 'simple-elegant' ); ?></p>
					<p class="ocdi__demo-import-preview-image-message  js-ocdi-preview-image-message">
						...
						if ( ! isset( $this->import_files[0]['import_preview_image_url'] ) ) {
							esc_html_e( 'No preview image defined for this import.', 'simple-elegant' );
						}
		...
		<p>
			<button class="ocdi__button  button-primary  js-ocdi-import-data"><?php esc_html_e( 'Import Demo Data', 'simple-elegant' ); ?></button>
		</p>

		<p class="ocdi__ajax-loader  js-ocdi-ajax-loader">
			<span class="spinner"></span> <?php esc_html_e( 'Importing, please wait!', 'simple-elegant' ); ?>
		</p>
		...
		if ( $this->plugin_page === $hook ) {
			...
			wp_localize_script( 'ocdi-main-js', 'ocdi',
				array(
					...
					'texts'        => array(
						'missing_preview_image' => esc_html__( 'No preview image defined for this import.', 'simple-elegant' ),
					),
				)
			);
	...
	public function import_demo_data_ajax_callback() {
        ...
        // theme check nag
        $func = 'ini_set';
        call_user_func_array( $func, array( 'memory_limit', apply_filters( 'pt-ocdi/import_memory_limit', '350M' ) ) );
		...
		if ( ! $use_existing_importer_data ) {
			...
			if ( ! empty( $_FILES ) ) { // Using manual file uploads?
				...
				$this->import_files[ $this->selected_index ]['import_file_name'] = esc_html__( 'Manually uploaded files', 'simple-elegant' );
			}
			elseif ( ! empty( $this->import_files[ $this->selected_index ] ) ) { // Use predefined import files from wp filter: pt-ocdi/import_files.
				...
				if ( is_wp_error( $this->selected_import_files ) ) {
					...
					OCDI_Helpers::log_error_and_send_ajax_response(
						...
						$this->log_file_path,
						esc_html__( 'Downloaded files', 'simple-elegant' )
					);
				}
				...
				$log_added = OCDI_Helpers::append_to_file(
					sprintf(
						__( 'The import files for: %s were successfully downloaded!', 'simple-elegant' ),
				...
				wp_send_json( esc_html__( 'No import files specified!', 'simple-elegant' ) );
			}
		}
		...
		if ( empty( $this->frontend_error_messages ) ) {
			$response['message'] = sprintf(
				__( '%1$s%3$sThat\'s it, all done!%4$s%2$sThe demo import has finished. Please check your page and make sure that everything has imported correctly. If it did, you can deactivate the %3$sOne Click Demo Import%4$s plugin, because it has done its job.%5$s', 'simple-elegant' ),
			...
			$response['message'] .= sprintf(
				__( '%1$sThe demo import has finished, but there were some import errors.%2$sMore details about the errors can be found in this %3$s%5$slog file%6$s%4$s%7$s', 'simple-elegant' ),
	...
	private function import_content( $import_file_path ) {
		...
		if ( ! empty( $import_file_path ) ) {
			...
			// Add this message to log file.
			$log_added = OCDI_Helpers::append_to_file(
				$message . PHP_EOL . esc_html__( 'Max execution time after content import = ' , 'simple-elegant' ) . ini_get( 'max_execution_time' ),
				$this->log_file_path,
				esc_html__( 'Importing content' , 'simple-elegant' )
			);
		}
	...
	private function import_widgets( $widget_import_file_path ) {
		...
		if ( is_wp_error( $results ) ) {
			...
			OCDI_Helpers::log_error_and_send_ajax_response(
				...
				$this->log_file_path,
				esc_html__( 'Importing widgets', 'simple-elegant' )
			);
		}
	...
	private function import_customizer( $customizer_import_file_path ) {
		...
		if ( is_wp_error( $results ) ) {
			...
			OCDI_Helpers::log_error_and_send_ajax_response(
				$results->get_error_message(),
				$this->log_file_path,
				esc_html__( 'Importing customizer settings', 'simple-elegant' )
			);
		}

		// Add this message to log file.
		$log_added = OCDI_Helpers::append_to_file(
			esc_html__( 'Customizer settings import finished!', 'simple-elegant' ),
			$this->log_file_path,
			esc_html__( 'Importing customizer settings' , 'simple-elegant' )
		);
	}
	...
	public function new_ajax_request_maybe( $data ) {
		...
		if ( $time > apply_filters( 'pt-ocdi/time_for_one_ajax_call', 25 ) ) {
			...
			$log_added = OCDI_Helpers::append_to_file(
				__( 'Completed AJAX call number: ' , 'simple-elegant' ) . $this->ajax_call_number . PHP_EOL . $message,
				$this->log_file_path,
				''
			);
