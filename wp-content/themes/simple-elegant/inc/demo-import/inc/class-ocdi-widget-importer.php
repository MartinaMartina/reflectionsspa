...
class OCDI_Widget_Importer {
	...
	private function process_import_file( $file ) {
		...
		if ( ! file_exists( $file ) ) {
			return new WP_Error(
				...
				__( 'Widget import file could not be found.', 'simple-elegant' )
			);
		}
	...
	private function import_data( $data ) {
		...
		if ( empty( $data ) || ! is_object( $data ) ) {
			return new WP_Error(
				...
				__( 'Widget import data could not be read. Please try a different file.', 'simple-elegant' )
			);
		}
		...
		foreach ( $data as $sidebar_id => $widgets ) {
			...
			if ( isset( $wp_registered_sidebars[ $sidebar_id ] ) ) {
				...
				$sidebar_message      = __( 'Sidebar does not exist in theme (moving widget to Inactive)', 'simple-elegant' );
			}
			...
			foreach ( $widgets as $widget_instance_id => $widget ) {
				...
				if ( ! $fail && ! isset( $available_widgets[ $id_base ] ) ) {
					...
					$widget_message      = __( 'Site does not support widget', 'simple-elegant' ); // Explain why widget not imported.
				}
				...
				if ( ! $fail && isset( $widget_instances[ $id_base ] ) ) {
					...
					foreach ( $single_widget_instances as $check_id => $check_widget ) {
						...
						if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
							...
							$widget_message      = __( 'Widget already exists', 'simple-elegant' ); // Explain why widget not imported.
				...
				if ( ! $fail ) {
					...
					if ( $sidebar_available ) {
						...
						$widget_message      = __( 'Imported', 'simple-elegant' );
					}
					else {
						$widget_message_type = 'warning';
						$widget_message      = __( 'Imported to Inactive', 'simple-elegant' );
					}
				}

				// Result for widget instance.
				$results[ $sidebar_id ]['widgets'][ $widget_instance_id ]['name']         = isset( $available_widgets[ $id_base ]['name'] ) ? $available_widgets[ $id_base ]['name'] : $id_base; // Widget name or ID if name not available (not supported by site).
				$results[ $sidebar_id ]['widgets'][ $widget_instance_id ]['title']        = ! empty( $widget['title'] ) ? $widget['title'] : __( 'No Title', 'simple-elegant' ); // Show "No Title" if widget instance is untitled.
	...
	public function format_results_for_log( $results ) {
		...
		if ( empty( $results ) ) {
			esc_html_e( 'No results for widget import!', 'simple-elegant' );
		}
